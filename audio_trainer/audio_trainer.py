...
def _set_logger(log_dir, verbose_level=1):
    ...
    # base logger
    log_file_path = Path(log_dir) /"LLM.log"
...
# with open(r'D:\Search\LLM\unaligned.pkl', 'rb') as f:
#     data = pickle.load(f)
...
class trainer():
    ...
    def do_train(self, train_dataloader, val_dataloader):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            path = "middleModel" + str(save_time) + ".pth"
            torch.save(self.model.state_dict(), path)
            logging.info(f"save model in {path}")
...
if __name__ == "__main__":
    args = {
        ...
        "num_epochs": 20,
        "batch_size": 16,
        "file_path": 'D:\Search\LLM\FT\\unaligned.pkl',
        "n_mels": 128,
        "n_audio_ctx": 1500,
        "n_audio_state": 1280,
        "n_audio_head": 20,
        ...
        "save_path": "middleModel",
        "load_path": "middleModel5.pth",

    }
    _set_logger(log_dir='D:\Search\LLM\whisper-main')
    args = edict(args)
    file_path = '/unaligned.pkl'
    ...
    # save result to csv
    csv_file = Path(f"D:\Search\LLM\whisper-main\\result.csv")
    if csv_file.is_file():
